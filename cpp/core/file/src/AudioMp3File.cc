...
size_t AudioMp3File::skipID3Header(std::fstream &file)
{
    ...
    size_t size = (static_cast<size_t>(header[6] & 0x7F) << 21) | (static_cast<size_t>(header[7] & 0x7F) << 14) |
                  (static_cast<size_t>(header[8] & 0x7F) << 7) | (static_cast<size_t>(header[9] & 0x7F));
